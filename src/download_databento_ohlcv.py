...
def normalize_ohlcv(df_raw: pd.DataFrame, local_tz: str) -> pd.DataFrame:
    ...
    # Fixed-point guard (same as your other script)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser("Download Databento OHLCV for a date range.")
    p.add_argument("--dataset", default="GLBX.MDP3")
    p.add_argument("--symbol", default="NQ.c.0")
    p.add_argument("--stype-in", default="continuous")
    p.add_argument("--schema", default="ohlcv-1m", help='Databento schema (default: "ohlcv-1m")')
    ...
    p.add_argument(
        "--out",
        default="data/databento/nq_ohlcv_1m_10y.parquet",
...
def main() -> int:
    ...
    ohlcv = normalize_ohlcv(raw, args.local_tz)
    save_ohlcv(ohlcv, out_path)
